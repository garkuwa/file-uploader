...
app.post("/upload", (req, res) => {
  ...
  if (!fs.existsSync(dataPath)) {
    ...
    cp.execSync(
      `mkdir -p ${dataPath} && cd ${dataPath} && unzip ${archivePath}`
    );
  }
...
app.get("/browse", (req, res) => {
  ...
  if (req.query.file) {
    ...
    if (fs.existsSync(filePath)) {
      res.send(sanitizer.value(fs.readFileSync(filePath, "utf-8")));
      return;
    }
  }
  ...
  files.forEach((f) => {
    if (!f.isDirectory()) {
      out += `<li><a href="/browse?md5=${md5}&file=${encodeURIComponent(
        f.name
      )}">${f.name}</a></li>`;
    }
  });
